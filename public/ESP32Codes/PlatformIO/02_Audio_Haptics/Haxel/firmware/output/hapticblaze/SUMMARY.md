...
| `API_SPEC.md` | REST `/json/*`, WebSocket `/ws` (subprotocol `haxel.v1`), WLED-compatible `/win` shim, mDNS discovery. |
...
│   ├── app.js
│   └── styles.css
├── include/Haxel.h       umbrella header
└── src/
...
- Wi-Fi AP `Haxel-XXXX` comes up reliably.
