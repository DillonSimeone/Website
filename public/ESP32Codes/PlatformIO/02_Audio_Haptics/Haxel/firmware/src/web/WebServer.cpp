...
public:
    ...
    bool canHandle(AsyncWebServerRequest *request) const override {
        ...
        if (host != "192.168.4.1" && host != "haxel.local" && host != config_->hostname() + ".local") {
            return true;
        }
        return false;
    }
...
void WebServer::mountCaptiveProbes_() {
    ...
    server_->onNotFound([](AsyncWebServerRequest* req) {
        ...
        // If the URL is already root, redirecting to root would cause an infinite redirection loop.
        // This typically happens if index.html is missing in LittleFS.
        if (url == "/" || url == "/index.html") {
            req->send(404, "text/html",
                      "<html><head><title>Haxel Portal Error</title>"
                      "<style>body{font-family:sans-serif;background:#121212;color:#fff;text-align:center;padding:50px;}"
                      "h1{color:#ff6a3d;}</style></head><body>"
                      "<h1>Haxel Portal Error</h1>"
                      "<p>Error 404: Web UI files not found in LittleFS filesystem.</p>"
                      "<p>Please upload the data partition (LittleFS image) to the ESP32.</p>"
                      "</body></html>");
            return;
        }
