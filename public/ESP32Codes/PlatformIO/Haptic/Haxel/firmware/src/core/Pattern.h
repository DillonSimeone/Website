...
public:
    ...
    // Optionally returns a static/custom color for the pattern on LEDs.
    // If it returns false, the controller will randomize the color.
    virtual bool getColor(uint8_t& r, uint8_t& g, uint8_t& b) { (void)r; (void)g; (void)b; return false; }
