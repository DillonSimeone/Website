# HapticBlaze Custom Network & Captive Portal Setup

This document details the configuration for HapticBlaze's offline wireless networking, captive portal redirection, and passwordless open Access Point (AP) setup.
...
- **Implementation**: Located in `generateApSsid_()` in [Config.cpp](file:///f:/Github/Website/public/ESP32Codes/PlatformIO/Haptic/HapticBlaze/firmware/src/core/Config.cpp#L35-41):
  ```cpp
  ...
  ```
- **How it works**: It reads the ESP32's unique 48-bit MAC address from hardware eFuses via `ESP.getEfuseMac()`, extracts the final 4 hexadecimal characters, and appends them to the SSID (e.g. `HapticBlaze-3CE7`). This allows you to flash dozens of boards without confusing which board you are connecting to.
...
- **Passwordless Open Network**: In [main.cpp](file:///f:/Github/Website/public/ESP32Codes/PlatformIO/Haptic/HapticBlaze/firmware/src/main.cpp#L105-112), we call the `WiFi.softAP()` method *without* passing a password argument:
  ```cpp
...
- **Implementation** in [CaptivePortal.cpp](file:///f:/Github/Website/public/ESP32Codes/PlatformIO/Haptic/HapticBlaze/firmware/src/web/CaptivePortal.cpp#L6-11):
  ```cpp
...
- In [WebServer.cpp](file:///f:/Github/Website/public/ESP32Codes/PlatformIO/Haptic/HapticBlaze/firmware/src/web/WebServer.cpp#L12-40), we register a custom `CaptiveRequestHandler` as the **very first handler** in the routing chain.
