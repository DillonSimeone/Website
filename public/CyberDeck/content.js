...
export class ContentProjector {
    ...
    _renderCanvas() {
        ...
        for (let i = 0; i < this.items.length; i++) {
            ...
            if (currentY < -20 || currentY > h + 40) {
                currentY += (isObject && item.expanded) ? 120 : 45;
                continue;
            }

            if (isHovered) {
                ctx.fillStyle = 'rgba(51, 255, 51, 0.15)';
                ctx.fillRect(20, currentY - 28, w - 40, 40);
                ctx.fillStyle = '#33ff33';
            } else {
                ctx.fillStyle = text.startsWith('──') ? 'rgba(255, 255, 255, 0.4)' : 'rgba(255, 255, 255, 0.8)';
            }
            
            ctx.fillText(text.startsWith('──') ? text : `› ${text}`, 35, currentY);

            if (isObject && item.expanded) {
                currentY += 35;
        ...
        // Scrollbar
        this.maxScroll = Math.max(0, currentY - offsetY - h);
        if (this.maxScroll > 0) {
