	...
	//If n, delete storage and start on page 1.
	...
	function loadPage(page, num2)
	{
		if(num != 0)
		{
			...
			let loop = true;
			let identity = parseInt(num2) + 1; //num2 is the ID of the div with the story and whatever in it. Don't worry about memorizing what num2 is, it's only used in this section.
			do
			{
				if(exists(identity))
				{
			...
			div.id = id; //This is how the deletion loop is able to find the correct divs to delete.
			...
			select.name = id; //This is how the ID is passed onto the loadPage so the deletion loop can find the right divs and remove them. Creative, huh?
		...
		//The deletion function. This is very alike to JQuery's .remove() function, expect that it only works with IDs instead of both Id and Class, which probably speeds up the program by a bit.
		function remove(ids)
			{
