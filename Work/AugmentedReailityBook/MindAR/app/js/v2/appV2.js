 ...
 * Modern ESM implementation with MindAR 1.2.5
 */
...
(function () {
    ...
    function createShadowPlane() {
        ...
        const m = new THREE.MeshBasicMaterial({ color: 0xff00b4, opacity: 0.1, transparent: true });
    ...
    function buildDev() {
        ...
        devConsole.innerHTML = `
            ...
            <!-- SYSTEM TUNER (Stability) -->
            <div style="margin-top:20px; border-top: 1px solid #333; padding-top: 10px;">
               <div style="color:#00f0ff;font-size:10px;font-weight:800;letter-spacing:0.1em;margin-bottom:8px;">SYSTEM TUNER</div>
               <div class="ctrl"><label>FilterMinCF</label><input type="range" id="dfmin" min="-6" max="0" step="0.1" value="-4"><span class="v" id="dfminv">0.0001</span></div>
            </div>

            <!-- SHADOW TUNER SECTION -->
            <div style="margin-top:20px; border-top: 1px solid #333; padding-top: 10px;">
                ...
                <div style="display:flex; justify-content:space-between; margin-bottom:10px;">
                    <div style="display:flex; gap:6px; align-items:center;">
                        <input type="checkbox" id="dlshowh" checked> <label for="dlshowh" style="font-size:10px;">DEBUG BOX</label>
                    </div>
                    <div style="display:flex; gap:6px; align-items:center;">
        ...
        ['ds', 'dpx', 'dpy', 'dpz', 'drx', 'dry', 'drz', 'dlerp'].forEach(id => {
        ...
        if (sl.fmin) {
            sl.fmin.oninput = () => {
                ...
                document.getElementById('dfminv').textContent = val.toFixed(6);
                if (mindarThree && mindarThree.controller) {
                    mindarThree.controller.filterMinCF = val;
                }
            };
        }
        ...
        if (copyBtn) {
            copyBtn.onclick = () => { 
                const st = pageStates.get(activePageIndex); 
                if (st) navigator.clipboard.writeText(JSON.stringify(st.config, null, 2)).then(() => alert('Copied!')); 
            };
        }

        if (devToggle) devToggle.onclick = () => { 
            devOpen = !devOpen; 
            devConsole.classList.toggle('open', devOpen); 
    ...
    function showVals() {
        ...
        set('dpxv', st.config.offsetX.toFixed(2));
        set('dpyv', st.config.offsetY.toFixed(2));
        set('dpzv', st.config.offsetZ.toFixed(2));
        set('drxv', st.config.rotationX);
        set('dryv', st.config.rotationY);
        set('drzv', st.config.rotationZ);
    }

    async function init() {
        console.log("APP V2: INITIALIZING MODERN STACK...");
        ...
        try {
            ...
            console.error("APP V2: Failed to load pages.json", e);
            return;
        }

        // Initialize MindAR 1.2.5
        mindarThree = new MindARThree({
            container: arContainer,
            imageTargetSrc: './targets/book_targets.mind',
            maxTrack: 1, 
            filterMinCF: 0.0001, 
            filterBeta: 0.1,
            uiLoading: 'no', 
        ...
        // --- Stability Buffer: Anchor Padding ---
        // We add extra anchors to prevent the worker from crashing on out-of-bounds indices
        // if it detects noise that it maps to a non-existent target.
        for (let j = pages.length; j < pages.length + 10; j++) {
            mindarThree.addAnchor(j);
        }

        pages.forEach((p, i) => {
            const anchor = mindarThree.addAnchor(i);
            scene.add(anchor.group); // Crucial: anchor.group must be in scene
            ...
            const state = {
                ...
                isTracking: false,
                smoother: new PoseSmoothing({ lerpAlpha: 0.1 }),
    ...
    startBtn.onclick = async () => {
        ...
        renderer.setAnimationLoop(() => {
            ...
            if (globalLight && lightEstimator) {
                const uiInt = getVal('lint', 4.0);
                const uiBias = getVal('lbias', -0.0001);
                const uiNear = getVal('lnear', 0.1);
                const uiManual = sl.lmanual?.checked || false;
                const uiMX = getVal('lmx', 0);
                const uiMY = getVal('lmy', 5);

                let intensity = uiInt;
                if (!uiManual) intensity = (lightEstimator.currentIntensity / 2.0) * uiInt;
            ...
            pageStates.forEach(state => {
                ...
                // Visibility Logic
                if (state.debugCube) state.debugCube.visible = uiShowH;
                ...
                if (state.isTracking) {
                    ...
                    const rawP = new THREE.Vector3(); 
                    const rawQ = new THREE.Quaternion(); 
                    const rawS = new THREE.Vector3();
