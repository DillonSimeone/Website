...
def get_svg_data(filename):
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        # Extract original width and height to create a viewBox
        ...
        # Strip existing width/height and add viewBox + full scale attributes
        # We also ensure it's flavored for JS string escaping
...
desktop_svg = get_svg_data('bg-desktop.svg')
mobile_svg = get_svg_data('bg-mobile.svg')

js_template = """/* 
  Static Asset Background - Optimized for zero-computation rendering.
  Fixed: Added viewBox and explicit scaling to ensure it fills the viewport.
*/
...
let initalHeight = window.innerHeight;
...
function draw() {
    ...
    // Inject SVG from static data string
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = svgData;
    const pattern = tempDiv.querySelector('svg');
    if (!pattern) return;

    // Re-skin with random colors once on load
...
final_js = js_template.replace("{{DESKTOP_SVG}}", desktop_svg).replace("{{MOBILE_SVG}}", mobile_svg)

with open('background.js', 'w', encoding='utf-8') as f:
    f.write(final_js)

print("background.js rebuilt successfully with proper scaling attributes.")
