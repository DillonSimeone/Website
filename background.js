...
function draw() {
    ...
    */
    ...
    document.addEventListener('mousemove', document.fn = function fn(e) {
            ...
            //Gets the pointer's x and y coords.
            const center = {
                x: e.clientX,
                y: e.clientY
            };
            //Defines where the the circle needs to go for its center to be right over the pointer's x and y coords.
            const circleCenter = {
                x: center.x - radius,
                y: center.y - radius
            };
            //Moves the circle to the pointer's postion.
            circle.setAttribute('style', 'transform: translate(' +
                circleCenter.x + 'px, ' +
                circleCenter.y + 'px)');
            for (let i = polyPoints.length - 1; i; i--) {
