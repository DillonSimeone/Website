...
void setMotorDirection(MotorMode mode, int duty) {
  switch (mode) {
    case MODE_OFF:
      ledcWrite(MOTOR_PIN_FWD, 0);
      ledcWrite(MOTOR_PIN_REV, 0);
      break;
    case MODE_FORWARD:
      ledcWrite(MOTOR_PIN_FWD, duty);
      ledcWrite(MOTOR_PIN_REV, 0);
      break;
    case MODE_REVERSE:
      ledcWrite(MOTOR_PIN_FWD, 0);
      ledcWrite(MOTOR_PIN_REV, duty);
      break;
...
void setup() {
  ...
  // These calls will now take over the pins we set earlier, which is perfectly fine.
  ledcAttach(NOODLE_LED_PIN, PWM_FREQ, PWM_BITS);
  ledcWrite(NOODLE_LED_PIN, 0);
  ledcAttach(MOTOR_PIN_FWD, PWM_FREQ, PWM_BITS);
  ledcWrite(MOTOR_PIN_FWD, 0);
  ledcAttach(MOTOR_PIN_REV, PWM_FREQ, PWM_BITS);
  ledcWrite(MOTOR_PIN_REV, 0);
...
void loop() {
  ...
  if (bytesRead > 0) {
    ...
    ledcWrite(NOODLE_LED_PIN, noodleDuty);
