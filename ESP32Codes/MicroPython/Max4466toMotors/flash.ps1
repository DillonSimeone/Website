# flash.ps1 - PowerShell script to flash MicroPython to ESP32-C3 SuperMini using PlatformIO's esptool

$ErrorActionPreference = "Stop"

$FIRMWARE_FILE = "ESP32_GENERIC_C3-20251209-v1.27.0.bin"
$FIRMWARE_PATH = Join-Path $PSScriptRoot $FIRMWARE_FILE
$BAUD_RATE = 460800

Write-Host "========================================================"
Write-Host "     ESP32-C3 SuperMini MicroPython Flasher"
Write-Host "========================================================"
Write-Host ""
Write-Host "Ensure your ESP32-C3 SuperMini is connected and powered on."
Write-Host ""
Write-Host "To enter DOWNLOAD MODE (if auto-reset fails):"
...
Write-Host "[1/4] Locating esptool.py..."
try {
...
Write-Host "[2/4] Detecting ESP32-C3 COM port..."
try {
...
# 3. Erase Flash
Write-Host "[3/4] Erasing flash (this may take a while)..."
try {
...
# 4. Flash Firmware
Write-Host "[4/4] Flashing firmware '$FIRMWARE_FILE'..."
try {
    & python $pioEsptoolPath --chip esp32c3 --port $esp32c3Port --baud $BAUD_RATE write_flash -z 0x0 $FIRMWARE_PATH
    if ($LASTEXITCODE -ne 0) { throw "esptool.py write_flash command failed." }
...
Write-Host ""
Write-Host "========================================================"
Write-Host "      SUCCESS! MicroPython Flashing Complete"
Write-Host "========================================================"
Write-Host ""
Write-Host "1. Press the RESET button on your board."
Write-Host "2. Connect to the board's serial port (e.g., via Thonny or 'pio device monitor -p $esp32c3Port') at 115200 baud."
Write-Host ""
pause
