# HapticBlaze — API Specification

HapticBlaze exposes three coordinated surfaces:
...
- Base URL on STA: `http://hapticblaze.local` (or chosen hostname)
...
- All numeric fields clamp on the server; out-of-range values produce a warning header `X-HapticBlaze-Warning` but a 200 response.
...
Subprotocol: `hapticblaze.v1` (clients SHOULD send this in `Sec-WebSocket-Protocol`; the server is lenient if absent).
...
- Service: `_hapticblaze._tcp` on port 80.
- TXT records: `version`, `driver`, `channels`, `actuator`.
- For WLED tool compatibility, the device **also** advertises `_wled._tcp` on the same port with TXT `compat=hapticblaze` so WLED scanners find it; the WLED app will then see "HapticBlaze (compat)" in its picker.
...
```bash
curl -X POST http://hapticblaze.local/json/state \
...
```js
const ws = new WebSocket('ws://hapticblaze.local/ws', 'hapticblaze.v1');
...
`/json/info.version` follows SemVer. Breaking REST or WS changes bump major. The `hapticblaze.v1` WebSocket subprotocol is the negotiation point — `v2` will be added alongside `v1` and `v1` will be supported for at least one major release.
