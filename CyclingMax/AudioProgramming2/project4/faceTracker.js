// Import Mediapipe Tasks and d3-delaunay for Voronoi generation.
import vision from "https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.3";
// Extract FaceLandmarker and FilesetResolver from vision.
const { FaceLandmarker, FilesetResolver } = vision;
import { Delaunay } from "https://cdn.jsdelivr.net/npm/d3-delaunay@6/+esm";
// Import HandLandmarker (latest version).
...
let handStates = [
    { isPinching: false, grabbedPieceId: null, lastPinchPos: { x: 0, y: 0, z: 0 }, lastUpdate: 0 },
    { isPinching: false, grabbedPieceId: null, lastPinchPos: { x: 0, y: 0, z: 0 }, lastUpdate: 0 }
  ];
  ...
  if (lastVideoTime !== video.currentTime) {
    ...
    if (faceResult.faceLandmarks && faceResult.faceLandmarks.length > 0) {
      ...
      for (let i = 0; i < absSeeds.length; i++) {
        ...
        let newPoly = clipped.map(p => ({ x: p[0], y: p[1], z: 0 }));
      ...
      if (pinchDetected) {
        ...
        if (!handStates[hIndex].isPinching || handStates[hIndex].grabbedPieceId === null) {
          ...
          if (candidate !== null) {
            ...
            candidate.lastHandPos = { ...newPinchPos };
            // Record the offset from the pinch position to the polygon’s centroid.
            const cent = computeCentroid(candidate.polygon);
            candidate.dragOffset = { dx: cent.x - newPinchPos.x, dy: cent.y - newPinchPos.y };
            candidate.initialPinchZ = newPinchPos.z;
            candidate.originalPolygon = candidate.polygon.map(pt => ({ ...pt }));
            // Mark as detached.
...
function sendPolygonLocations() {
  const data = maskPieces.map((piece, idx) => {
    ...
    return { id: idx, x: cent.x, y: cent.y, z: zVal, attached: piece.attached };
  });
...
function computeCentroid(polygon) {
  ...
  return { x: sumX / polygon.length, y: sumY / polygon.length, z: sumZ / polygon.length };
}
